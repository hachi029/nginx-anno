 ...
 * 
 * 是一个filter, 解压上游带有“Content-Encoding: gzip”  以应对浏览器不支持gzip的场景
 * 
 ...
 */
  ...
  */
typedef struct {
    ngx_flag_t           enable;        //标识是否开启了本模块
    ngx_bufs_t           bufs;          //  gunzip_buffers number size; 解压缩时使用的缓存数量和大小
...
ngx_http_gunzip_header_filter(ngx_http_request_t *r)
{
    ...
    //如果未开启gunzip 或 响应头 content_encoding 不是gzip
    if (!conf->enable
    ...
    //清除content_length和accept_ranges请求头
...
/**
 * 将ctx->in上的第一个buf挂到ctx->zstream.next_in链表上
 */
static ngx_int_t
    ...
    ngx_http_gunzip_ctx_t *ctx)
{
    ...
    } else if (ctx->bufs < conf->bufs.num) {        //仍有可用buf
